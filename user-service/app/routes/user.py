...
def signin(payload: UserSignIn, db=Depends(get_db)):
...
def update_existing_user(user_id: str, user: UserUpdate, db=Depends(get_db)):
...
def delete_existing_user(user_id: str, db=Depends(get_db)):
